...
function mealImage(meal) {
  ...
  if (/яхния|чили|гювеч|леща|свободен слот/.test(txt)) return base + "chicken-stew.webp";
  if (/банан и|диня|пъпеш|кашу|ядки|грозде|орехи|смокин|царевиц/.test(txt)) return base + "fruit-nuts.webp";
...
function viewToday() {
  ...
  // час на тренировката: изрично зададен в програмата (w.at), иначе по типа —
  // кардиото е веднага след ставане, залата/домашната тренировка след блока за умения
  const isCardio = w && /кардио/i.test(w.t);
  const wTime = w ? (w.at || (isCardio ? "06:05" : "07:00")) : null;
  const skillTime = (wTime === "06:05") ? "07:00" : "06:05";
  const items = [];
  items.push({ id:"wake", time:"06:00", t:"Ставане + вода", s:"Телефонът остава далеч първите 30 мин", k:"habit" });
  if (w) items.push({ id:"__workout", time:wTime, t:`Тренировка · ${w.t}`, s:`${w.dur} мин · ${P}/${N} упражнения`, k:"train", nav:["plan","train"], done:workoutDone });
  items.push({ id:"skill", time:skillTime, t:workday?"Умения / AI проекти":"Умения / AI проекти · сутрешен блок", k:"habit" });
      ...
      return h("div",{class:`card meal ${o[`meal${a}`]?"done":""}`,"data-scroll":`meal${a}`},
        ...
        h("button",{class:"mealHead","aria-expanded":openMeal===a?"true":"false",onclick:()=>{openMeal=openMeal===a?null:a;render();}},
          h("div",{class:"mealHeadL"},
            ...
            h("div",{class:"chips"},
              e.mac?h("span",{class:"chip mac"},e.mac):null,
              e.dev.map((t)=>h("span",{class:"chip"},t)),
              bothOn?h("span",{class:"chip hot"},"за двамата 👩"):null),
...
const EX_DESC = {
  ...
  /* ---- зала (програма v1, август) ---- */
  "Гоблет клек":"Дъмбел или пудовка на гърдите с две ръце, лактите вътре. Седни назад и надолу с изправен торс, тежестта в петите. Тежестта отпред пази кръста неутрален — затова е вместо клек с щанга.",
  "Лег преса":"Стъпалата на ширина на раменете в средата на платформата. Спускай контролирано до ~90° в коляното. Кръстът и таза остават ЗАЛЕПЕНИ за облегалката — не пускай коленете до гърдите.",
  "Румънска тяга с дъмбели (леки)":"Дъмбелите пред бедрата, леко свити колене. Избутай таза назад и свали дъмбелите по бедрата с НЕУТРАЛЕН гръб. Усещаш опъване в задното бедро, не в кръста. При дискомфорт — спираш веднага.",
  "Румънска тяга с дъмбели":"Движение от тазобедрената става, а не от кръста. Гърбът е права дъска през целия път, дъмбелите се плъзгат близо до бедрата.",
  "Планк":"Опора на предмишници и пръсти, тяло в права линия. Стегни корем и седалище. Ако кръстът провисне — прекрати серията.",
  "Дъмбел преса на лег":"Легнал, дъмбелите на нивото на гърдите, лактите под ~45° спрямо тялото. Избутай нагоре и леко навътре, без да чукаш дъмбелите.",
  "Раменна преса с дъмбели (седнал с облегалка)":"Облегалката е задължителна — тя сваля натоварването от кръста. Дъмбелите от нивото на ушите нагоре, без извиване на гърба назад.",
  "Флайс на машина (pec deck)":"Лактите леко свити и фиксирани. Събирай с гърдите, не с ръцете; в края задръж половин секунда стискане.",
  "Трицепс разгъване на скрипец":"Лактите прибрани до тялото и НЕПОДВИЖНИ. Само предмишницата се движи — надолу до пълно изпъване, бавно нагоре.",
  "Придърпване на горен скрипец":"Хват малко по-широк от раменете. Дърпай лоста към горната част на гърдите, свивайки лопатките надолу и назад. Без залюляване с тялото.",
  "Гребане с опора на гърди":"Гърдите опрени в подложката — така кръстът изобщо не участва. Дърпай лактите назад покрай тялото и стискай лопатките.",
  "Face pull на скрипец":"Въжето на нивото на лицето. Дърпай към челото с разтваряне на лактите настрани — упражнение за здрави рамене и стойка.",
  ...
  "Напади с дъмбели (или гоблет клек)":"Крачка назад, задното коляно почти до пода, торсът изправен. Връщаш се през предната пета. При дискомфорт в коляното — смени с гоблет клек.",
  "Гребане на долен скрипец седнал":"Гърбът изправен, коленете леко свити. Дърпай дръжката към корема, лопатките назад. НЕ се залюлявай с кръста напред-назад.",
  "Dead bug":"Легнал по гръб, ръце нагоре, колене на 90°. Спусни противоположна ръка и крак почти до пода — кръстът остава притиснат към пода през цялото време.",
};
...
const SHOP_ICONS = [
  ...
  ["🍄", ["гъби"]],
  ["🧅", ["лук","чесън","магданоз"]],
  ["🍑", ["праскови","кайси","нектарин","смокин"]],
  ["🍌", ["банан"]],
  ["🥑", ["авокадо"]],
...
function viewShop() {
  ...
  return h("section",null,
    ...
    (m.shopTrips || SHOP_TRIPS).map((trip) => {
